	...
	: m_assigned(0)
{
	...
	m_last_name = std::string("Last Name");
	m_nickname = std::string("Nickname");
	m_phone_number = std::string("Phone Num");
	m_darkest_secret = std::string("Secret");
}
...
void Contact::formatDisplay(int index)
{
	...
	// Last Name
	std::cout << std::setfill(' ') << std::setw(10) << format(m_last_name);
	std::cout << std::resetiosflags(std::ios::showbase) << "|";
